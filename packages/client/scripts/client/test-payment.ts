...
import { callProtectedEndpoint, getBalance } from '../../lib/client-wallet';
...
async function main() {
  ...
  if (Number.parseFloat(balance.eth) < 0.001) {
  ...
  try {
    ...
    if (paymentInfo) {
      ...
      if (paymentInfo.transaction) {
        console.log(
          `View on explorer: https://sepolia.basescan.org/tx/${paymentInfo.transaction}`
        );
      }
    }
